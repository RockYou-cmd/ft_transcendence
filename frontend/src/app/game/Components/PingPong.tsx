...
export default function PingPong({map, ballColor, paddleColor, PLAYER1, PLAYER2} : {PLAYER1 : string, PLAYER2 : string , map : string, ballColor : string, paddleColor : string}){
	...
	useEffect(() => {
		...
		let player1 = {x: 20, y: gameHeight / 2 - 120 /2 , score: 0, username: "", width: 12, height : 140, color : paddleColor};
		let player2 = {x: gameWidth -32, y: gameHeight /2 -120 /2, score: 0, username: "", width: 12, height : 140, color : paddleColor};
		let ball = {x: 0, y: 0, color : ballColor, radius : 16 };
		let net={x: gameWidth/2 - 2/1, y: 0, width: 2, height: 10, color: "white"};
		...
		socket?.on("frame", (data : any) => {
			...
			ball.x = data.ball.x;
			ball.y = data.ball.y;
			if (myScore != data.player1.score)
				setMyScore(data.player1.score);
			if (oppScore != data.player2.score)
