	...
	return (
		...
		<div className="m-8 flex flex-row gap-8 h-[85vh] " >
			...
			<div ref={mousehover} className='gap-8 w-full grid  grid-cols-3 grid-rows-4 ' id={hover ? "profile_grid_blur" : ""} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} >
				...
				<div className="rounded-lg  w-[30%] bg-black/5 min-w-full overflow-hidden hover:ease-in-out hover:scale-105 duration-700 shadow-lg shadow-gray-500 " id={hover ? "child" : ""}>3
		...
		return (
			<div className=" flex flex-col rounded-lg  items-center  h-full min-w-[400px]  bg-gradient-to-r from-black/25 to-black/80" >profile info
				...
				<div className='grid grid-cols-2 grid-rows-2 gap-3 w-[95%] h-fit '>
					<div className='text-2xl col-span-2 bg-blue-700/50 flex justify-center items-center h-16 rounded-tr-2xl rounded-tl-2xl'>
						<Image src={achiev_pic} width={60} height={60} alt="achievment" />  Achievmnet
					</div>
					<div className='bg-red-500 h-auto'>1</div>
					<div className='bg-red-500 h-auto'>2</div>
					<div className='bg-red-500 h-auto'>3</div>
					<div className='bg-red-500 h-auto'>4</div>
				</div>
			</div>
		)
	}
